...
public class Account {
     ...
     * @throws NegativeAmountException    if amount is negative, or balance is not sufficient
     *                      or ssn is not valid.
    ...
    public void withdraw(double amount, String ssn)
            throws NegativeAmountException, InsufficientBalanceException, SssNotValidException {
        try {
            if (amount < 0) {
                throw new NegativeAmountException("To ποσό " + amount + " είναι αρνητικό");
            }
            ...
            if (!isSsnValid(ssn)) {
                throw new SssNotValidException("Το ssn " + ssn + " δεν είναι έγκυρο");
            }
            balance -= amount;
        } catch (NegativeAmountException | InsufficientBalanceException | SssNotValidException e) {
            System.err.println(LocalDateTime.now() + "\n" + e);
           // e.printStackTrace(); // βλέπω που υπάρχει το λάθος.
            throw e;
        }
    }
    ...
    private boolean isSsnValid(String ssn) { // δεν παρέχεται σαν υπηρεσία προς τον client.
